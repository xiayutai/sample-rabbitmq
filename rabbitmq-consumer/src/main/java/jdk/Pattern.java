        ...
        implements java.io.Serializable
{
     ...
     */
    transient List<java.util.regex.Pattern.Node> topClosureNodes;
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (pattern.isEmpty()) {
            root = new java.util.regex.Pattern.Start(lastAccept);
    ...
    private void compile() {
        ...
        buffer = new int[32];
        groupNodes = new java.util.regex.Pattern.GroupHead[10];
        namedGroups = null;
        ...
        if (matchRoot instanceof java.util.regex.Pattern.Slice) {
            root = java.util.regex.Pattern.BnM.optimize(matchRoot);
            if (root == matchRoot) {
                root = hasSupplementary ? new java.util.regex.Pattern.StartS(matchRoot) : new java.util.regex.Pattern.Start(matchRoot);
            }
        } else if (matchRoot instanceof java.util.regex.Pattern.Begin || matchRoot instanceof java.util.regex.Pattern.First) {
            root = matchRoot;
        } else {
            root = hasSupplementary ? new java.util.regex.Pattern.StartS(matchRoot) : new java.util.regex.Pattern.Start(matchRoot);
        }
        ...
        if (!hasGroupRef) {
            for (java.util.regex.Pattern.Node node : topClosureNodes) {
                if (node instanceof java.util.regex.Pattern.Loop) {
                    // non-deterministic-greedy-group
                    ((java.util.regex.Pattern.Loop)node).posIndex = localTCNCount++;
                }
            }
        }
     ...
     */
    private java.util.regex.Pattern.Node expr(java.util.regex.Pattern.Node end) {
        java.util.regex.Pattern.Node prev = null;
        java.util.regex.Pattern.Node firstTail = null;
        java.util.regex.Pattern.Branch branch = null;
        java.util.regex.Pattern.Node branchConn = null;

        for (;;) {
            java.util.regex.Pattern.Node node = sequence(end);
            java.util.regex.Pattern.Node nodeTail = root;      //double return
            if (prev == null) {
                prev = node;
                firstTail = nodeTail;
            } else {
                // Branch
                if (branchConn == null) {
                    branchConn = new java.util.regex.Pattern.BranchConn();
                    branchConn.next = end;
                }
                if (node == end) {
                ...
                if (prev == branch) {
                    ...
                    prev = branch = new java.util.regex.Pattern.Branch(prev, node, branchConn);
                }
            }
            if (peek() != '|') {
                return prev;
     ...
     */
    private java.util.regex.Pattern.Node sequence(java.util.regex.Pattern.Node end) {
        java.util.regex.Pattern.Node head = null;
        java.util.regex.Pattern.Node tail = null;
        java.util.regex.Pattern.Node node = null;
        LOOP:
        for (;;) {
            int ch = peek();
            switch (ch) {
                ...
                case '[':
                    if (has(CANON_EQ) && !has(LITERAL))
                        node = new java.util.regex.Pattern.NFCCharProperty(clazz(true));
                    else
                ...
                case '\\':
                    ...
                    if (ch == 'p' || ch == 'P') {
                        ...
                        if (has(CANON_EQ) && !has(LITERAL))
                            node = new java.util.regex.Pattern.NFCCharProperty(family(oneLetter, comp));
                        else
                            node = newCharProperty(family(oneLetter, comp));
                    } else {
                        unread();
                ...
                case '^':
                    ...
                    if (has(MULTILINE)) {
                        if (has(UNIX_LINES))
                            node = new java.util.regex.Pattern.UnixCaret();
                        else
                            node = new java.util.regex.Pattern.Caret();
                    } else {
                        node = new java.util.regex.Pattern.Begin();
                    }
                    break;
                case '$':
                    next();
                    if (has(UNIX_LINES))
                        node = new java.util.regex.Pattern.UnixDollar(has(MULTILINE));
                    else
                        node = new java.util.regex.Pattern.Dollar(has(MULTILINE));
                    break;
                case '.':
                    next();
                    if (has(DOTALL)) {
                        node = new java.util.regex.Pattern.CharProperty(ALL());
                    } else {
                        if (has(UNIX_LINES)) {
                            node = new java.util.regex.Pattern.CharProperty(UNIXDOT());
                        } else {
                            node = new java.util.regex.Pattern.CharProperty(DOT());
                        }
                    }
                    break;
                case '|':
                    ...
                    if (ch == 'p' || ch == 'P') { // Property
                        if (first > 0) { // Slice is waiting; handle it first
                            ...
                            if (ch != '{')
                                ...
                                oneLetter = false;
                            if (has(CANON_EQ) && !has(LITERAL))
                                return new java.util.regex.Pattern.NFCCharProperty(family(oneLetter, comp));
                            else
     ...
     */
    private java.util.regex.Pattern.Node ref(int refNum) {
        boolean done = false;
        while(!done) {
            int ch = peek();
            switch(ch) {
        ...
        if (has(CASE_INSENSITIVE))
            return new java.util.regex.Pattern.CIBackRef(refNum, has(UNICODE_CASE));
        else
            return new java.util.regex.Pattern.BackRef(refNum);
    }
    ...
    private int escape(boolean inclass, boolean create, boolean isrange) {
        ...
        switch (ch) {
            ...
            case 'A':
                if (inclass) break;
                if (create) root = new java.util.regex.Pattern.Begin();
                return -1;
            case 'B':
                if (inclass) break;
                if (create) root = new java.util.regex.Pattern.Bound(java.util.regex.Pattern.Bound.NONE, has(UNICODE_CHARACTER_CLASS));
                return -1;
            case 'C':
                break;
            case 'D':
            ...
            case 'G':
                if (inclass) break;
                if (create) root = new java.util.regex.Pattern.LastMatch();
                return -1;
            case 'H':
                if (create) {
            ...
            case 'R':
                if (inclass) break;
                if (create) root = new java.util.regex.Pattern.LineEnding();
                return -1;
            case 'S':
                if (create) {
                    predicate = has(UNICODE_CHARACTER_CLASS) ?
            ...
            case 'X':
                if (inclass) break;
                if (create) {
                    root = new java.util.regex.Pattern.XGrapheme();
                }
                return -1;
            case 'Y':
                break;
            case 'Z':
                ...
                if (create) {
                    if (has(UNIX_LINES))
                        root = new java.util.regex.Pattern.UnixDollar(false);
                    else
                        root = new java.util.regex.Pattern.Dollar(false);
                }
                return -1;
            case 'a':
                return '\007';
            case 'b':
                ...
                if (create) {
                    if (peek() == '{') {
                        if (skip() == 'g') {
                            if (read() == '}') {
                                root = new java.util.regex.Pattern.GraphemeBound();
                                return -1;
                            }
                    ...
                    root = new java.util.regex.Pattern.Bound(java.util.regex.Pattern.Bound.BOTH, has(UNICODE_CHARACTER_CLASS));
                }
                return -1;
            case 'c':
                return c();
            ...
            case 'k':
                ...
                if (create) {
                    hasGroupRef = true;
                    if (has(CASE_INSENSITIVE))
                        root = new java.util.regex.Pattern.CIBackRef(namedGroups().get(name), has(UNICODE_CASE));
                    else
                        root = new java.util.regex.Pattern.BackRef(namedGroups().get(name));
                }
                return -1;
            case 'l':
            case 'm':
            ...
            case 'z':
                if (inclass) break;
                if (create) root = new java.util.regex.Pattern.End();
                return -1;
            default:
                return ch;
        }
     ...
     */
    private java.util.regex.Pattern.CharPredicate clazz(boolean consume) {
        java.util.regex.Pattern.CharPredicate prev = null;
        java.util.regex.Pattern.CharPredicate curr = null;
        java.util.regex.Pattern.BitClass bits = new java.util.regex.Pattern.BitClass();
        java.util.regex.Pattern.BmpCharPredicate bitsP = ch -> ch < 256 && bits.bits[ch];
        ...
        for (;;) {
            switch (ch) {
                ...
                case '&':
                    ...
                    if (ch == '&') {
                        ch = next();
                        java.util.regex.Pattern.CharPredicate right = null;
                        while (ch != ']' && ch != '&') {
                            if (ch == '[') {
                                if (right == null)
                                    right = clazz(true);
    ...
    private java.util.regex.Pattern.CharPredicate bitsOrSingle(java.util.regex.Pattern.BitClass bits, int ch) {
     ...
     */
    private java.util.regex.Pattern.CharPredicate single(final int ch) {
        if (has(CASE_INSENSITIVE)) {
            int lower, upper;
            if (has(UNICODE_CASE)) {
     ...
     */
    private java.util.regex.Pattern.CharPredicate range(java.util.regex.Pattern.BitClass bits) {
        int ch = peek();
        if (ch == '\\') {
            ch = nextEscaped();
     ...
     */
    private java.util.regex.Pattern.CharPredicate family(boolean singleLetter, boolean isComplement) {
        next();
        String name;
        java.util.regex.Pattern.CharPredicate p = null;
    ...
    private java.util.regex.Pattern.CharProperty newCharProperty(java.util.regex.Pattern.CharPredicate p) {
        if (p == null)
            return null;
        if (p instanceof java.util.regex.Pattern.BmpCharPredicate)
            return new java.util.regex.Pattern.BmpCharProperty((java.util.regex.Pattern.BmpCharPredicate)p);
        else
            return new java.util.regex.Pattern.CharProperty(p);
    }
    ...
    private java.util.regex.Pattern.Node group0() {
        ...
        if (ch == '?') {
            ...
            switch (ch) {
                ...
                case '!':
                    ...
                    head.next = expr(tail);
                    if (ch == '=') {
                        head = tail = new java.util.regex.Pattern.Pos(head);
                    } else {
                        head = tail = new java.util.regex.Pattern.Neg(head);
                    }
                    break;
                case '>':   // (?>xxx)  independent group
                    ...
                    tail = root;
                    head.next = expr(tail);
                    head = tail = new java.util.regex.Pattern.Ques(head, java.util.regex.Pattern.Qtype.INDEPENDENT);
                    break;
                case '<':   // (?<xxx)  look behind
                    ...
                    java.util.regex.Pattern.TreeInfo info = new java.util.regex.Pattern.TreeInfo();
                    head.study(info);
                    ...
                    if (ch == '=') {
                        head = tail = (hasSupplementary ?
                                new java.util.regex.Pattern.BehindS(head, info.maxLength,
                                        info.minLength) :
                                new java.util.regex.Pattern.Behind(head, info.maxLength,
                                        info.minLength));
                    } else { // if (ch == '!')
                        head = tail = (hasSupplementary ?
                                new java.util.regex.Pattern.NotBehindS(head, info.maxLength,
                                        info.minLength) :
                                new java.util.regex.Pattern.NotBehind(head, info.maxLength,
                                        info.minLength));
                    }
        ...
        java.util.regex.Pattern.Node node = closure(head);
        ...
        if (node instanceof java.util.regex.Pattern.Ques) {
            java.util.regex.Pattern.Ques ques = (java.util.regex.Pattern.Ques) node;
            if (ques.type == java.util.regex.Pattern.Qtype.POSSESSIVE) {
                root = node;
                return node;
            }
            tail.next = new java.util.regex.Pattern.BranchConn();
            tail = tail.next;
            if (ques.type == java.util.regex.Pattern.Qtype.GREEDY) {
                head = new java.util.regex.Pattern.Branch(head, null, tail);
            } else { // Reluctant quantifier
                head = new java.util.regex.Pattern.Branch(null, head, tail);
            }
            root = tail;
            return head;
        } else if (node instanceof java.util.regex.Pattern.Curly) {
            java.util.regex.Pattern.Curly curly = (java.util.regex.Pattern.Curly) node;
            if (curly.type == java.util.regex.Pattern.Qtype.POSSESSIVE) {
                root = node;
                return node;
            }
            ...
            if (head.study(info)) { // Deterministic
                java.util.regex.Pattern.GroupTail temp = (java.util.regex.Pattern.GroupTail) tail;
                head = root = new java.util.regex.Pattern.GroupCurly(head.next, curly.cmin,
                        curly.cmax, curly.type,
                        ((java.util.regex.Pattern.GroupTail)tail).localIndex,
                        ((java.util.regex.Pattern.GroupTail)tail).groupIndex,
                        capturingGroup);
                return head;
            } else { // Non-deterministic
                int temp = ((java.util.regex.Pattern.GroupHead) head).localIndex;
                java.util.regex.Pattern.Loop loop;
                if (curly.type == java.util.regex.Pattern.Qtype.GREEDY) {
                    loop = new java.util.regex.Pattern.Loop(this.localCount, temp);
                    // add the max_reps greedy to the top-closure-node list
                    if (curly.cmax == MAX_REPS)
                        topClosureNodes.add(loop);
                } else {  // Reluctant Curly
                    loop = new java.util.regex.Pattern.LazyLoop(this.localCount, temp);
                }
                java.util.regex.Pattern.Prolog prolog = new java.util.regex.Pattern.Prolog(loop);
                this.localCount += 1;
     ...
     */
    private java.util.regex.Pattern.Node createGroup(boolean anonymous) {
        int localIndex = localCount++;
        int groupIndex = 0;
        if (!anonymous)
            groupIndex = capturingGroupCount++;
        java.util.regex.Pattern.GroupHead head = new java.util.regex.Pattern.GroupHead(localIndex);
        root = new java.util.regex.Pattern.GroupTail(localIndex, groupIndex);

        // for debug/print only, head.match does NOT need the "tail" info
        head.tail = (java.util.regex.Pattern.GroupTail)root;
    ...
    private java.util.regex.Pattern.Node curly(java.util.regex.Pattern.Node prev, int cmin) {
        int ch = next();
        if (ch == '?') {
            next();
            return new java.util.regex.Pattern.Curly(prev, cmin, MAX_REPS, java.util.regex.Pattern.Qtype.LAZY);
        } else if (ch == '+') {
            next();
            return new java.util.regex.Pattern.Curly(prev, cmin, MAX_REPS, java.util.regex.Pattern.Qtype.POSSESSIVE);
        }
        if (prev instanceof java.util.regex.Pattern.BmpCharProperty) {
            return new java.util.regex.Pattern.BmpCharPropertyGreedy((java.util.regex.Pattern.BmpCharProperty)prev, cmin);
        } else if (prev instanceof java.util.regex.Pattern.CharProperty) {
            return new java.util.regex.Pattern.CharPropertyGreedy((java.util.regex.Pattern.CharProperty)prev, cmin);
        }
        return new java.util.regex.Pattern.Curly(prev, cmin, MAX_REPS, java.util.regex.Pattern.Qtype.GREEDY);
    }
     ...
     */
    private java.util.regex.Pattern.Node closure(java.util.regex.Pattern.Node prev) {
        ...
        switch (ch) {
            case '?':
                ...
                if (ch == '?') {
                    next();
                    return new java.util.regex.Pattern.Ques(prev, java.util.regex.Pattern.Qtype.LAZY);
                } else if (ch == '+') {
                    next();
                    return new java.util.regex.Pattern.Ques(prev, java.util.regex.Pattern.Qtype.POSSESSIVE);
                }
                return new java.util.regex.Pattern.Ques(prev, java.util.regex.Pattern.Qtype.GREEDY);
            case '*':
                return curly(prev, 0);
            case '+':
                return curly(prev, 1);
            case '{':
                ...
                if (ASCII.isDigit(ch)) {
                    ...
                    java.util.regex.Pattern.Curly curly;
                    ch = peek();
                    if (ch == '?') {
                        next();
                        curly = new java.util.regex.Pattern.Curly(prev, cmin, cmax, java.util.regex.Pattern.Qtype.LAZY);
                    } else if (ch == '+') {
                        next();
                        curly = new java.util.regex.Pattern.Curly(prev, cmin, cmax, java.util.regex.Pattern.Qtype.POSSESSIVE);
                    } else {
                        curly = new java.util.regex.Pattern.Curly(prev, cmin, cmax, java.util.regex.Pattern.Qtype.GREEDY);
                    }
                    return curly;
                } else {
     ...
     */
    static final class BitClass extends java.util.regex.Pattern.BmpCharProperty {
        final boolean[] bits;
        BitClass() {
        ...
        java.util.regex.Pattern.BitClass add(int c, int flags) {
            assert c >= 0 && c <= 255;
     ...
     */
    private java.util.regex.Pattern.Node newSlice(int[] buf, int count, boolean hasSupplementary) {
        ...
        if (has(CASE_INSENSITIVE)) {
            if (has(UNICODE_CASE)) {
                ...
                return hasSupplementary? new java.util.regex.Pattern.SliceUS(tmp) : new java.util.regex.Pattern.SliceU(tmp);
            }
            for (int i = 0; i < count; i++) {
                tmp[i] = ASCII.toLower(buf[i]);
            }
            return hasSupplementary? new java.util.regex.Pattern.SliceIS(tmp) : new java.util.regex.Pattern.SliceI(tmp);
        }
        for (int i = 0; i < count; i++) {
            tmp[i] = buf[i];
        }
        return hasSupplementary ? new java.util.regex.Pattern.SliceS(tmp) : new java.util.regex.Pattern.Slice(tmp);
    }
    ...
    static class Node extends Object {
         ...
         */
        boolean study(java.util.regex.Pattern.TreeInfo info) {
            if (next != null) {
    ...
    static class Start extends java.util.regex.Pattern.Node {
        int minLength;
        Start(java.util.regex.Pattern.Node node) {
            this.next = node;
            java.util.regex.Pattern.TreeInfo info = new java.util.regex.Pattern.TreeInfo();
            next.study(info);
    ...
    static final class StartS extends java.util.regex.Pattern.Start {
        StartS(java.util.regex.Pattern.Node node) {
            super(node);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            if (i > matcher.to - minLength) {
     ...
     */
    static final class LineEnding extends java.util.regex.Pattern.Node {
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
     ...
     * boolean property.
     */
    static class CharProperty extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.CharPredicate predicate;

        CharProperty (java.util.regex.Pattern.CharPredicate predicate) {
     ...
     */
    private static class BmpCharProperty extends java.util.regex.Pattern.CharProperty {
        BmpCharProperty (java.util.regex.Pattern.BmpCharPredicate predicate) {
            super(predicate);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
    ...
    private static class NFCCharProperty extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.CharPredicate predicate;
        NFCCharProperty (java.util.regex.Pattern.CharPredicate predicate) {
            this.predicate = predicate;
        }
     ...
     */
    static class XGrapheme extends java.util.regex.Pattern.Node {
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                int ch0 = Character.codePointAt(seq, i);
     ...
     */
    static class GraphemeBound extends java.util.regex.Pattern.Node {
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int startIndex = matcher.from;
            int endIndex = matcher.to;
     ...
     * characters.
     */
    static class Slice extends java.util.regex.Pattern.SliceNode {
        Slice(int[] buf) {
            super(buf);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
     ...
     */
    static final class SliceS extends java.util.regex.Pattern.Slice {
        SliceS(int[] buf) {
            super(buf);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
    ...
    static final class Ques extends java.util.regex.Pattern.Node {
        ...
        Ques(java.util.regex.Pattern.Node node, java.util.regex.Pattern.Qtype type) {
            this.atom = node;
            this.type = type;
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
        ...
        boolean study(java.util.regex.Pattern.TreeInfo info) {
            if (type != java.util.regex.Pattern.Qtype.INDEPENDENT) {
     ...
     */
    static class CharPropertyGreedy extends java.util.regex.Pattern.Node {
        final java.util.regex.Pattern.CharPredicate predicate;
        final int cmin;

        CharPropertyGreedy(java.util.regex.Pattern.CharProperty cp, int cmin) {
    ...
    static final class BmpCharPropertyGreedy extends java.util.regex.Pattern.CharPropertyGreedy {

        BmpCharPropertyGreedy(java.util.regex.Pattern.BmpCharProperty bcp, int cmin) {
            super(bcp, cmin);
        }
    ...
    static final class Curly extends java.util.regex.Pattern.Node {
        ...
        Curly(java.util.regex.Pattern.Node node, int cmin, int cmax, java.util.regex.Pattern.Qtype type) {
            this.atom = node;
            this.type = type;
            this.cmin = cmin;
            this.cmax = cmax;
        ...
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            ...
            if (type == java.util.regex.Pattern.Qtype.GREEDY)
                return match0(matcher, i, j, seq);
            else if (type == java.util.regex.Pattern.Qtype.LAZY)
    ...
    static final class GroupCurly extends java.util.regex.Pattern.Node {
        ...
        GroupCurly(java.util.regex.Pattern.Node node, int cmin, int cmax, java.util.regex.Pattern.Qtype type, int local,
    ...
    static final class Branch extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node[] atoms = new java.util.regex.Pattern.Node[2];
        int size = 2;
        java.util.regex.Pattern.Node conn;
        Branch(java.util.regex.Pattern.Node first, java.util.regex.Pattern.Node second, java.util.regex.Pattern.Node branchConn) {
            conn = branchConn;
            atoms[0] = first;
            atoms[1] = second;
        }
        ...
        void add(java.util.regex.Pattern.Node node) {
            if (size >= atoms.length) {
                java.util.regex.Pattern.Node[] tmp = new java.util.regex.Pattern.Node[atoms.length*2];
        ...
        boolean study(java.util.regex.Pattern.TreeInfo info) {
            ...
            for (int n = 0; n < size; n++) {
                ...
                if (atoms[n] != null)
                    atoms[n].study(info);
                minL2 = java.lang.Math.min(minL2, info.minLength);
                maxL2 = java.lang.Math.max(maxL2, info.maxLength);
    ...
    static final class GroupHead extends java.util.regex.Pattern.Node {
        int localIndex;
        java.util.regex.Pattern.GroupTail tail;    // for debug/print only, match does not need to know
    ...
    static final class GroupRef extends java.util.regex.Pattern.Node {
        ...
        GroupRef(java.util.regex.Pattern.GroupHead head) {
            this.head = head;
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
    ...
    static final class Prolog extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Loop loop;
        Prolog(java.util.regex.Pattern.Loop loop) {
            this.loop = loop;
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
    ...
    static final class First extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node atom;
        First(java.util.regex.Pattern.Node node) {
            this.atom = java.util.regex.Pattern.BnM.optimize(node);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            if (atom instanceof java.util.regex.Pattern.BnM) {
    ...
    static final class Conditional extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node cond, yes, not;
        Conditional(java.util.regex.Pattern.Node cond, java.util.regex.Pattern.Node yes, java.util.regex.Pattern.Node not) {
            this.cond = cond;
            this.yes = yes;
            this.not = not;
        }
        ...
        boolean study(java.util.regex.Pattern.TreeInfo info) {
            ...
            info.minLength = minL + java.lang.Math.min(minL2, info.minLength);
            info.maxLength = maxL + java.lang.Math.max(maxL2, info.maxLength);
    ...
    static final class Pos extends java.util.regex.Pattern.Node {
        ...
        Pos(java.util.regex.Pattern.Node cond) {
            this.cond = cond;
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
    ...
    static final class Neg extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node cond;
        Neg(java.util.regex.Pattern.Node cond) {
            this.cond = cond;
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int savedTo = matcher.to;
     ...
     * was encountered.
     */
    static java.util.regex.Pattern.Node lookbehindEnd = new java.util.regex.Pattern.Node() {
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
    ...
    static class Behind extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node cond;
        int rmax, rmin;
        Behind(java.util.regex.Pattern.Node cond, int rmax, int rmin) {
            this.cond = cond;
            this.rmax = rmax;
            this.rmin = rmin;
        }
        ...
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            ...
            int startIndex = (!matcher.transparentBounds) ?
                    matcher.from : 0;
            int from = java.lang.Math.max(i - rmax, startIndex);
            // Set end boundary
    ...
    static final class BehindS extends java.util.regex.Pattern.Behind {
        BehindS(java.util.regex.Pattern.Node cond, int rmax, int rmin) {
        ...
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            ...
            int startIndex = (!matcher.transparentBounds) ?
                    matcher.from : 0;
            boolean conditionMatched = false;
            int from = java.lang.Math.max(i - rmaxChars, startIndex);
            // Set end boundary
            int savedLBT = matcher.lookbehindTo;
            matcher.lookbehindTo = i;
            // Relax transparent region boundaries for lookbehind
    ...
    static class NotBehind extends java.util.regex.Pattern.Node {
        java.util.regex.Pattern.Node cond;
        int rmax, rmin;
        NotBehind(java.util.regex.Pattern.Node cond, int rmax, int rmin) {
            this.cond = cond;
            this.rmax = rmax;
            this.rmin = rmin;
        }
    ...
    static final class NotBehindS extends java.util.regex.Pattern.NotBehind {
        NotBehindS(java.util.regex.Pattern.Node cond, int rmax, int rmin) {
            super(cond, rmax, rmin);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int rmaxChars = countChars(seq, i, -rmax);
    ...
    static class BnM extends java.util.regex.Pattern.Node {
        ...
        static java.util.regex.Pattern.Node optimize(java.util.regex.Pattern.Node node) {
            if (!(node instanceof java.util.regex.Pattern.Slice)) {
                return node;
            }

            int[] src = ((java.util.regex.Pattern.Slice) node).buffer;
            ...
            if (node instanceof java.util.regex.Pattern.SliceS)
                return new java.util.regex.Pattern.BnMS(src, lastOcc, optoSft, node.next);
            return new java.util.regex.Pattern.BnM(src, lastOcc, optoSft, node.next);
        }
        BnM(int[] src, int[] lastOcc, int[] optoSft, java.util.regex.Pattern.Node next) {
            this.buffer = src;
            ...
            NEXT:       while (i <= last) {
                ...
                for (int j = patternLength - 1; j >= 0; j--) {
                    ...
                    if (ch != src[j]) {
                        ...
                        i += java.lang.Math.max(j + 1 - lastOcc[ch&0x7F], optoSft[j]);
                        continue NEXT;
                    }
                }
    ...
    static final class BnMS extends java.util.regex.Pattern.BnM {
        ...
        BnMS(int[] src, int[] lastOcc, int[] optoSft, java.util.regex.Pattern.Node next) {
            ...
            NEXT:       while (i <= last) {
                ...
                for (int j = countChars(seq, i, patternLength), x = patternLength - 1;
                    ...
                    if (ch != src[x]) {
                        // Shift search to the right by the maximum of the
                        // bad character shift and the good suffix shift
                        int n = java.lang.Math.max(x + 1 - lastOcc[ch&0x7F], optoSft[x]);
    ...
    static interface CharPredicate {
        ...
        default java.util.regex.Pattern.CharPredicate and(java.util.regex.Pattern.CharPredicate p) {
            return ch -> is(ch) && p.is(ch);
        }
        default java.util.regex.Pattern.CharPredicate union(java.util.regex.Pattern.CharPredicate p) {
            return ch -> is(ch) || p.is(ch);
        }
        default java.util.regex.Pattern.CharPredicate union(java.util.regex.Pattern.CharPredicate p1,
                                                            java.util.regex.Pattern.CharPredicate p2 ) {
            return ch -> is(ch) || p1.is(ch) || p2.is(ch);
        }
        default java.util.regex.Pattern.CharPredicate negate() {
            return ch -> !is(ch);
        }
    }

    static interface BmpCharPredicate extends java.util.regex.Pattern.CharPredicate {

        default java.util.regex.Pattern.CharPredicate and(java.util.regex.Pattern.CharPredicate p) {
            if(p instanceof java.util.regex.Pattern.BmpCharPredicate)
                return (java.util.regex.Pattern.BmpCharPredicate)(ch -> is(ch) && p.is(ch));
            return ch -> is(ch) && p.is(ch);
        }
        default java.util.regex.Pattern.CharPredicate union(java.util.regex.Pattern.CharPredicate p) {
            if (p instanceof java.util.regex.Pattern.BmpCharPredicate)
        ...
        static java.util.regex.Pattern.CharPredicate union(java.util.regex.Pattern.CharPredicate... predicates) {
            java.util.regex.Pattern.CharPredicate cp = ch -> {
                for (java.util.regex.Pattern.CharPredicate p : predicates) {
                    if (!p.is(ch))
                        return false;
                }
                return true;
            ...
            return (java.util.regex.Pattern.BmpCharPredicate)cp;
        }
    }
     ...
     */
    static java.util.regex.Pattern.CharPredicate SingleS(int c) {
        return ch -> ch == c;
    }
     ...
     */
    static java.util.regex.Pattern.BmpCharPredicate Single(int c) {
        return ch -> ch == c;
    }
     ...
     * Case insensitive matches a given BMP character
     */
    static java.util.regex.Pattern.BmpCharPredicate SingleI(int lower, int upper) {
     ...
     */
    static java.util.regex.Pattern.CharPredicate SingleU(int lower) {
        return ch -> lower == ch ||
     ...
     */
    static java.util.regex.Pattern.CharPredicate Range(int lower, int upper) {
        if (upper < Character.MIN_HIGH_SURROGATE ||
                lower > Character.MAX_HIGH_SURROGATE &&
                        upper < Character.MIN_SUPPLEMENTARY_CODE_POINT)
            return (java.util.regex.Pattern.BmpCharPredicate)(ch -> inRange(lower, ch, upper));
     ...
     */
    static java.util.regex.Pattern.CharPredicate CIRange(int lower, int upper) {
        return ch -> inRange(lower, ch, upper) ||
                ASCII.isAscii(ch) &&
    ...
    static java.util.regex.Pattern.CharPredicate CIRangeU(int lower, int upper) {
        return ch -> {
     ...
     */
    static final java.util.regex.Pattern.Node accept = new java.util.regex.Pattern.Node();

    static final java.util.regex.Pattern.Node lastAccept = new java.util.regex.Pattern.LastNode();
