...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    static final int MAX_ARRAY_SIZE = java.lang.Integer.MAX_VALUE - 8;
    ...
    private static final ObjectStreamField[] serialPersistentFields = {
            ...
            new ObjectStreamField("segmentMask", java.lang.Integer.TYPE),
            new ObjectStreamField("segmentShift", java.lang.Integer.TYPE),
    };
    ...
    private static final int tableSizeFor(int c) {
        int n = -1 >>> java.lang.Integer.numberOfLeadingZeros(c - 1);
    ...
    static final <K,V> java.util.concurrent.ConcurrentHashMap.Node<K,V> tabAt(java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab, int i) {
        return (java.util.concurrent.ConcurrentHashMap.Node<K,V>)U.getObjectAcquire(tab, ((long)i << ASHIFT) + ABASE);
    }
    ...
    public int size() {
        long n = sumCount();
        return ((n < 0L) ? 0 :
                (n > (long) java.lang.Integer.MAX_VALUE) ? java.lang.Integer.MAX_VALUE :
                        (int)n);
    }
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab = table;;) {
            java.util.concurrent.ConcurrentHashMap.Node<K,V> f; int n, i, fh; K fk; V fv;
            if (tab == null || (n = tab.length) == 0)
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
                if (casTabAt(tab, i, null, new java.util.concurrent.ConcurrentHashMap.Node<K,V>(hash, key, value)))
                    ...
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {
                            ...
                            for (java.util.concurrent.ConcurrentHashMap.Node<K,V> e = f;; ++binCount) {
                                ...
                                if ((e = e.next) == null) {
                                    pred.next = new java.util.concurrent.ConcurrentHashMap.Node<K,V>(hash, key, value);
                                    break;
                                }
                            }
                        }
                        ...
                        else if (f instanceof java.util.concurrent.ConcurrentHashMap.ReservationNode)
    ...
    public java.util.concurrent.ConcurrentHashMap.KeySetView<K,V> keySet() {
        ...
        return keySet = new java.util.concurrent.ConcurrentHashMap.KeySetView<K,V>(this, null);
    }
    ...
    public Collection<V> values() {
        ...
        return values = new java.util.concurrent.ConcurrentHashMap.ValuesView<K,V>(this);
    }
    ...
    public Set<Map.Entry<K,V>> entrySet() {
        ...
        return entrySet = new java.util.concurrent.ConcurrentHashMap.EntrySetView<K,V>(this);
    }
                    ...
                    if (casTabAt(tab, i, null, r)) {
                        ...
                        try {
                            if ((val = mappingFunction.apply(key)) != null)
                                node = new java.util.concurrent.ConcurrentHashMap.Node<K,V>(h, key, val);
                        } finally {
                            setTabAt(tab, i, node);
                        }
                    }
                                ...
                                if ((e = e.next) == null) {
                                    if ((val = mappingFunction.apply(key)) != null) {
                                        if (pred.next != null)
                                            throw new IllegalStateException("Recursive update");
                                        added = true;
                                        pred.next = new java.util.concurrent.ConcurrentHashMap.Node<K,V>(h, key, val);
                                    }
                                    break;
                                }
                            }
    ...
    public V merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
        ...
        for (java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                ...
                if (casTabAt(tab, i, null, new java.util.concurrent.ConcurrentHashMap.Node<K,V>(h, key, value))) {
                    delta = 1;
                    val = value;
                    break;
                }
    ...
    static final int resizeStamp(int n) {
        return java.lang.Integer.numberOfLeadingZeros(n) | (1 << (RESIZE_STAMP_BITS - 1));
    }
    ...
    private final void addCount(long x, int check) {
        java.util.concurrent.ConcurrentHashMap.CounterCell[] cs; long b, s;
        if ((cs = counterCells) != null ||
                !U.compareAndSetLong(this, BASECOUNT, b = baseCount, s = b + x)) {
            java.util.concurrent.ConcurrentHashMap.CounterCell c; long v; int m;
    ...
    final long sumCount() {
        java.util.concurrent.ConcurrentHashMap.CounterCell[] cs = counterCells;
        long sum = baseCount;
        if (cs != null) {
            for (java.util.concurrent.ConcurrentHashMap.CounterCell c : cs)
                if (c != null)
                    sum += c.value;
        }
        return sum;
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (;;) {
            java.util.concurrent.ConcurrentHashMap.CounterCell[] cs; java.util.concurrent.ConcurrentHashMap.CounterCell c; int n; long v;
    ...
    private final void treeifyBin(java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab, int index) {
        java.util.concurrent.ConcurrentHashMap.Node<K,V> b; int n;
        if (tab != null) {
    ...
    static <K,V> java.util.concurrent.ConcurrentHashMap.Node<K,V> untreeify(java.util.concurrent.ConcurrentHashMap.Node<K,V> b) {
        ...
        for (java.util.concurrent.ConcurrentHashMap.Node<K,V> q = b; q != null; q = q.next) {
            java.util.concurrent.ConcurrentHashMap.Node<K,V> p = new java.util.concurrent.ConcurrentHashMap.Node<K,V>(q.hash, q.key, q.val);
            if (tl == null)
                hd = p;
            else
                tl.next = p;
    ...
    static final class TreeBin<K,V> extends java.util.concurrent.ConcurrentHashMap.Node<K,V> {
        ...
        private static final long LOCKSTATE
                = U.objectFieldOffset(java.util.concurrent.ConcurrentHashMap.TreeBin.class, "lockState");
    }
    ...
    static final class KeyIterator<K,V> extends java.util.concurrent.ConcurrentHashMap.BaseIterator<K,V>
        ...
        KeyIterator(java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab, int size, int index, int limit,
                    java.util.concurrent.ConcurrentHashMap<K,V> map) {
    ...
    static final class ValueIterator<K,V> extends java.util.concurrent.ConcurrentHashMap.BaseIterator<K,V>
        ...
        ValueIterator(java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab, int size, int index, int limit,
                      java.util.concurrent.ConcurrentHashMap<K,V> map) {
            super(tab, size, index, limit, map);
        }
    ...
    static final class EntryIterator<K,V> extends java.util.concurrent.ConcurrentHashMap.BaseIterator<K,V>
        ...
        EntryIterator(java.util.concurrent.ConcurrentHashMap.Node<K,V>[] tab, int size, int index, int limit,
                      java.util.concurrent.ConcurrentHashMap<K,V> map) {
            super(tab, size, index, limit, map);
        }
    ...
    static final class MapReduceKeysToIntTask<K,V>
        ...
        public final java.lang.Integer getRawResult() { return result; }
        public final void compute() {
            final ToIntFunction<? super K> transformer;
            final IntBinaryOperator reducer;
            if ((transformer = this.transformer) != null &&
    ...
    static {
        SIZECTL = U.objectFieldOffset
                (java.util.concurrent.ConcurrentHashMap.class, "sizeCtl");
        TRANSFERINDEX = U.objectFieldOffset
                (java.util.concurrent.ConcurrentHashMap.class, "transferIndex");
        BASECOUNT = U.objectFieldOffset
                (java.util.concurrent.ConcurrentHashMap.class, "baseCount");
        CELLSBUSY = U.objectFieldOffset
                (java.util.concurrent.ConcurrentHashMap.class, "cellsBusy");

        CELLVALUE = U.objectFieldOffset
                (java.util.concurrent.ConcurrentHashMap.CounterCell.class, "value");

        ABASE = U.arrayBaseOffset(java.util.concurrent.ConcurrentHashMap.Node[].class);
        int scale = U.arrayIndexScale(java.util.concurrent.ConcurrentHashMap.Node[].class);
        if ((scale & (scale - 1)) != 0)
        ...
        ensureLoaded = java.util.concurrent.ConcurrentHashMap.ReservationNode.class;
    }
}
